...
> O alimentador de contexto definitivo e automatizado para o seu Agente de IA.
...
* 🔍 **Auto-Scan Automático (`scan`)**: A ferramenta lê o seu `package.json` (Node/JS/TS) ou `requirements.txt` (Python), identifica todas as suas dependências e versões, e gera um manual rigoroso para a IA não errar as versões do seu projeto.
...
```bash
git clone [https://github.com/SEU_USUARIO/brainfeed.git](https://github.com/SEU_USUARIO/brainfeed.git)
cd brainfeed